...
class ContractBase(BaseModel):
    ...
    remaining_hours: Decimal = Field(..., gte=0, description="剩余课时")
    unit_price: Decimal = Field(..., gte=0, description="单价(元/课时)")
    total_amount: Decimal = Field(..., gte=0, description="合同总金额")
    received_amount: Decimal = Field(default=Decimal("0.00"), gte=0, description="实收金额")
    discount_amount: Decimal = Field(default=Decimal("0.00"), gte=0, description="优惠金额")
...
class ContractUpdate(BaseModel):
    ...
    remaining_hours: Optional[Decimal] = Field(None, gte=0, description="剩余课时")
    unit_price: Optional[Decimal] = Field(None, gte=0, description="单价")
    total_amount: Optional[Decimal] = Field(None, gte=0, description="合同总金额")
    received_amount: Optional[Decimal] = Field(None, gte=0, description="实收金额")
    discount_amount: Optional[Decimal] = Field(None, gte=0, description="优惠金额")
